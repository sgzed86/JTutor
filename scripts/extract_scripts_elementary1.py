#!/usr/bin/env python3
"""
Extract speak/listen targets from Elementary 1 PDF dialog scripts
(A：/B： lines + CD markers like 01-04) → content/elementary1/script_extract.json

No Whisper — phrases come from the textbook.
"""
...
# Short speech bubbles / answers (avoid instructional ～ましょう lines)
MODEL_RE = re.compile(
    r"((?:お)?[\u3040-\u30ff\u4e00-\u9fffー]{2,16}"
...
def extract_lesson(reader: PdfReader, lesson_num: int, start: int, end: int) -> dict:
    ...
    for p in range(start, min(end, len(reader.pages)) + 1):
        ...
        # Prefer dialog lines for CD mapping; models are fallback only
...
def main() -> None:
    ...
    if not PDF_EXTRACT.exists():
        raise SystemExit("Run extract_pdf_elementary1.py first")
